        ...
        if ~isfile("./results/v02/SARSA/evaluation/$(params_str)/checkpoint_25.csv")
            ...
            for index in 1:5000
                @time begin
                    ...
                    run_episode(df[i:(i + day),:], "./results/v02/SARSA/", models, encoders, tuple_size, n_steps, epsilon, learning_rate, decay_rate, index)
                end
            end
            counter = 1
            for j in evaluation
                @time begin
                    i = (j-1)*day + 1
                    run_episode_eval(df[i:(i + day),:], "./results/v02/SARSA/evaluation/", models, encoders, tuple_size, n_steps, epsilon, learning_rate, decay_rate, counter)
                    counter = counter + 1
                end
            end
        else
            @info ("SKIP - $(params_str)")
        end
    # catch y
