...
function run_episode(df, path, vmodel::forget_model.Model, models::Array{policy_model.Model}, encoders::Dict{String, thermometer.Thermometer}, tuple_size::Int, n_steps::Int64, learning_rate::Float64, decay_rate::Float64, index::Int)
    ...
    while t_ < T_ - 1
        ...
        if t_ > 1
            ...
            if t_ + n_steps < T_
                G = G + learning_rate^n_steps * forget_model.predict(env.V, BinInput(env, S[t_]))
            end
            alpha = G - forget_model.predict(env.V, BinInput(env, S[t_]))
            ...
            for a in 1:length(env.fmodels)
                ...
                policy_model.train!(env.fmodels[a], BinInput(env, S[t_]), g)
            end
        end
        t = t + 1      
    end
...
function run_episode_eval(df, path, vmodel::forget_model.Model, models::Array{policy_model.Model}, encoders::Dict{String, thermometer.Thermometer}, tuple_size::Int, n_steps::Int64, learning_rate::Float64, decay_rate::Float64, index::Int)
    mypath = joinpath(path, "learning-rate=$(learning_rate)_decay-rate=$(decay_rate)_n-steps=$(n_steps)_tuple-size=$(tuple_size)")
    mkpath(mypath)
    filename = joinpath(mypath, "checkpoint_$(index).csv")
...
function run_episode(df, path, vmodel::forget_model.Model, models::Array{policy_model.Model}, encoders::Dict{String, thermometer.Thermometer}, tuple_size::Int, n_steps::Int64, learning_rate::Float64, decay_rate::Float64, input_size::Int, index::Int)
    mypath = joinpath(path, "learning-rate=$(learning_rate)_decay-rate=$(decay_rate)_n-steps=$(n_steps)_tuple-size=$(tuple_size)_encoder-size=$(input_size)")
    mkpath(mypath)
    filename = joinpath(mypath, "checkpoint_$(index).csv")
...
function get_enviroment(df, Vmodel::forget_model.Model, Fmodels::Array{policy_model.Model}, encoders::Dict{String, thermometer.Thermometer}, n_steps::Int64, learning_rate::Float64, decay_rate::Float64)
